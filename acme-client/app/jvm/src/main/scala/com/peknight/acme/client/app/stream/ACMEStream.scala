package com.peknight.acme.client.app.stream
...
import com.peknight.acme.client.app.context.StreamContext
...
import com.peknight.acme.client.stream.CertificatesStream
...
object ACMEStream:
  def apply[F[_]: {Async, Parallel, Logger, Files, Env}]: Resource[F, StreamContext[F]] =
    for
      ...
      certificates = CertificatesStream.persisted[F, Challenge, DNS, `dns-01`, DNSRecordId](
    ...
    yield
      StreamContext(config, client, acmeClient, dnsRecordApi, dnsChallengeClient, certificates, provider)
